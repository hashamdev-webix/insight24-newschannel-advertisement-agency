...
export default function Home() {
  ...
  const localArticles = getArticlesByCategory('local-news');
      ...
      <main className="flex-1">
              ...
              <Link href={`/articles/${featuredArticle.slug}`} className="group block">
                <div className="relative h-72 md:h-96 overflow-hidden bg-gray-200">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-5">
                    <span className="text-xs font-black bg-[#dd0000] text-white px-2 py-1 mr-2">
                      ● LIVE
                    </span>
        ...
        <section className="max-w-7xl mx-auto px-4 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0 border border-gray-200 border-t-0">
            {worldArticles.slice(0, 4).map((article, i) => (
              <Link
                key={article.id}
                href={`/articles/${article.slug}`}
                className={`group block p-4 hover:bg-gray-50 transition ${i < 3 ? 'border-r border-gray-200' : ''}`}
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-gray-200 border-t-0">
                {businessArticles.slice(0, 3).map((article, i) => (
                  <Link
                    key={article.id}
                    href={`/articles/${article.slug}`}
                    className={`group block p-4 hover:bg-gray-50 transition ${i < 2 ? 'border-r border-gray-200' : ''}`}
                  >
        ...
        {entertainmentArticles.length > 0 && (
          <section className="bg-[#f5f5f5] py-8 mb-0">
            <div className="max-w-7xl mx-auto px-4">
              <SectionHeader title="Entertainment" href="/category/entertainment" light />
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {entertainmentArticles.slice(0, 3).map((article) => (
                  <Link key={article.id} href={`/articles/${article.slug}`} className="group bg-white border border-gray-200 hover:shadow-md transition block">
        ...
        <section className="max-w-7xl mx-auto px-4 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0 border border-gray-200 border-t-0">
            {techArticles.slice(0, 4).map((article, i) => (
              <Link
                key={article.id}
                href={`/articles/${article.slug}`}
                className={`group block p-4 hover:bg-gray-50 transition ${i < 3 ? 'border-r border-gray-200' : ''}`}
            ...
            <div className="lg:col-span-3">
              ...
              {localArticles.length > 0 ? (
                <div className="divide-y divide-gray-200 border border-gray-200 border-t-0">
                  {localArticles.slice(0, 4).map((article) => (
                    <Link key={article.id} href={`/articles/${article.slug}`} className="group flex gap-4 p-4 hover:bg-gray-50 transition">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {healthArticles.slice(0, 3).map((article) => (
                    <Link key={article.id} href={`/articles/${article.slug}`} className="group flex gap-3">
        ...
        <section className="max-w-7xl mx-auto px-4 py-8 mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {articles.slice(15, 25).map((article) => (
              <Link key={article.id} href={`/articles/${article.slug}`} className="group block">
                <div className="relative h-28 overflow-hidden bg-gray-200 mb-2">
                  <Image src={article.image} alt={article.title} fill className="object-cover group-hover:scale-105 transition duration-300" />
                </div>
                <p className="text-[10px] font-bold text-[#dd0000] uppercase tracking-wider mb-0.5">{article.category}</p>
        ...
        <section className="bg-[#dd0000] py-12">
          <div className="max-w-4xl mx-auto px-4 text-center">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link
                href="/advertise"
                className="bg-white text-[#dd0000] font-black text-sm px-8 py-3 hover:bg-gray-100 transition uppercase tracking-widest"
              >
                Request Advertisement
              </Link>
              <Link
                href="/contact"
                className="border-2 border-white text-white font-bold text-sm px-8 py-3 hover:bg-white/10 transition uppercase tracking-widest"
              >
                Contact Us
              </Link>
            </div>
...
function SectionHeader({ title, href, light }: { title: string; href?: string; light?: boolean }) {
  return (
    <div className={`flex items-center justify-between border-b-4 border-[#dd0000] mb-0 pb-2 ${light ? '' : ''}`}>
